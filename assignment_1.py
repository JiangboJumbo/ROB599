...
def generate_edges(n, phi):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    cone_edges = np.array([[np.cos(2*np.pi*i/n)*np.cos(phi), np.sin(2*np.pi*i/n)*np.cos(phi), np.sin(phi)] for i in range(n)])
    # ------------------------------------------------
...
def compute_normals(cone_edges):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    n = np.shape(cone_edges)[0]
    cone_edges = np.append(cone_edges, [cone_edges[0]], axis=0)
    facet_normals = np.array([np.cross(cone_edges[i], cone_edges[i+1]) for i in range(n)])
    # ------------------------------------------------
...
def compute_minimum_distance_from_facet_normals(a, facet_normals):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    distance_list = [np.dot(normal, a)/np.linalg.norm(normal) for normal in facet_normals]
...
def check_is_interior_point(a, n, phi):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    normals = compute_normals(generate_edges(n, phi))
    within_list = [np.dot(normal, a)/np.linalg.norm(normal) >= 0 for normal in normals]
    outliers = within_list.count(False)
    is_interior_point = outliers == 0  # TODO: Replace None with your result
    # ------------------------------------------------
